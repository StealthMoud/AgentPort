...
func New(customRoot string) *CodexAdapter {
	return &CodexAdapter{customRoot: customRoot}
}
...
func (c *CodexAdapter) Detect(ctx context.Context) (*adapter.DetectResult, error) {
	...
	if err == nil && info.IsDir() {
		return &adapter.DetectResult{
			...
			Detected: true,
			RootPath: root,
			Message:  "Codex directory detected",
		}, nil
	}
	...
	return &adapter.DetectResult{
		...
		Detected: false,
		RootPath: root,
		Message:  "Codex installation not found",
	}, nil
}

func (c *CodexAdapter) Scan(ctx context.Context) (*adapter.ScanResult, error) {
...
func (c *CodexAdapter) PlanExportV2(ctx context.Context, manifest *adapter.CompileManifest) (*adapter.ExportPlan, error) {
	root, err := c.resolveRoot()
	if err != nil {
		return nil, err
	}
	...
	for _, art := range artifacts {
		if art.Kind != model.KindInstruction && art.Kind != model.KindMemory && art.Kind != model.KindPreference {
			plan.Items = append(plan.Items, &adapter.ExportItem{
				...
				Reason:           fmt.Sprintf("kind %s unsupported by codex export baseline", art.Kind),
			})
			continue
		}
...
func (c *CodexAdapter) ApplyExport(ctx context.Context, plan *adapter.ExportPlan) (*adapter.ExportResult, error) {
...
func (c *CodexAdapter) Validate(ctx context.Context) error {
	return nil
}
